...
import { Activity, Gauge, Map as MapIcon, History, Play, Square, BrainCircuit, AlertTriangle, ChevronRight, Settings, X, Key, Wrench, Shield, Eye, EyeOff } from 'lucide-react';
...
export default function App() {
  ...
  // Adapter responsiveness check
  useEffect(() => {
    if (connectionStatus !== 'connected' || isSimulation) return;
    
    const checkInterval = setInterval(() => {
      setConnectionMetadata(prev => {
        const silenceTime = prev.lastSeen ? Date.now() - prev.lastSeen : 0;
        const isResponding = silenceTime < 5000;
        if (prev.isResponding !== isResponding) {
          return { ...prev, isResponding };
        }
        return prev;
      });
    }, 2000);
  ...
  const connectToOBD = async () => {
    try {
      ...
      setConnectionMetadata(prev => ({ ...prev, error: undefined, deviceName: device?.name || 'Unknown OBD Device' }));
      ...
      setConnectionStatus('connected');
      setConnectionMetadata(prev => ({ ...prev, isResponding: true, lastSeen: Date.now() }));
      ...
      setConnectionMetadata(prev => ({ ...prev, error: message }));
  ...
  const parseOBDResponse = (buffer: string) => {
    setConnectionMetadata(prev => ({ ...prev, lastSeen: Date.now(), isResponding: true }));
  ...
  const connectToSerialOBD = async () => {
    try {
      ...
      setConnectionStatus('connecting');
      setConnectionMetadata(prev => ({ ...prev, error: undefined, deviceName: 'Serial OBD Adapter' }));
      setIsSimulation(false);
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 font-mono">
              <div className={cn(
                ...
                isSimulation ? "bg-car-warning" : 
                connectionStatus === 'connected' ? 
                  (connectionMetadata.isResponding === false ? "bg-car-danger animate-pulse" : "bg-car-success shadow-[0_0_8px_rgba(34,197,94,0.6)]") :
                connectionStatus === 'connecting' ? "bg-car-accent animate-ping" : "bg-white/20"
              )} />
              <p className="text-[8px] uppercase tracking-[0.2em] text-white/40">
                {isSimulation ? "Simulation Mode" : (connectionMetadata.deviceName || "Telemetry System")}
              </p>
            </div>
          </div>
        </div>
