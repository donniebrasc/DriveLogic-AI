...
export default function OBDTab({ data, isSimulation, connectionStatus, connectionMetadata, onConnectReal, onConnectSerial }: OBDTabProps) {
  ...
  const getStatusConfig = () => {
    ...
    switch (connectionStatus) {
      case 'connected':
        if (connectionMetadata.isResponding === false) {
          return { 
            label: 'Connection Frozen', 
            color: 'text-car-danger', 
            bg: 'bg-car-danger/10',
            dot: 'bg-car-danger animate-pulse',
            icon: AlertTriangle
          };
        }
        return { 
          label: connectionMetadata.deviceName || 'OBD-II Linked', 
      ...
      case 'connecting':
        return { 
          label: connectionMetadata.deviceName ? `Connecting ${connectionMetadata.deviceName}...` : 'Connecting...', 
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] opacity-60 font-mono">
                {connectionStatus === 'connecting' ? `Initializing link with ${connectionMetadata.deviceName || 'adapter'}...` : 
                 connectionStatus === 'connected' ? 
                  (connectionMetadata.isResponding === false ? 
                    'Adapter stopped sending data. Check proximity or hardware.' : 
                    `Receiving 10Hz telemetry from ${connectionMetadata.deviceName}`) : 
                 'Connect adapter to see real telemetry'}
              </p>
            </div>
          </div>
