...
import { Map as MapIcon, MapPin, Navigation, BrainCircuit, AlertCircle, Info, ArrowRight, Key, Search, AlertTriangle, Mic, Filter, Calendar, Ruler, History, ChevronDown, ChevronUp, Clock } from 'lucide-react';
...
export default function GPSTab({ isRecording, trips, navigation, setNavigation, mapsApiKey }: GPSTabProps) {
  ...
  const filteredTrips = trips.filter(trip => {
    ...
    if (trip.distance < filters.minDist) return false;
    if (trip.distance > filters.maxDist) return false;
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 rounded-3xl space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-car-accent/10 rounded-xl">
              <History className="text-car-accent" size={20} />
            </div>
            <div>
              <h3 className="font-bold text-white">Trip Explorer</h3>
              <p className="text-xs text-white/40">Analyze past routes</p>
            </div>
          </div>
          <button
            ...
            className={cn(
              ...
              showFilters ? "bg-car-accent text-white border-car-accent" : "bg-white/5 text-white/40 border-white/10"
            )}
          >
            <Filter size={18} />
          </button>
        ...
        {showFilters && (
          <motion.div 
            ...
            className="grid grid-cols-2 gap-4 p-4 bg-white/5 rounded-2xl border border-white/10"
          >
              ...
              <div className="space-y-4 pt-2">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={(e) => setFilters({...filters, minDist: parseInt(e.target.value)})}
                    className="w-full accent-car-accent"
                  />
                </div>
                <div className="space-y-1">
                  <div className="flex justify-between text-[8px] text-white/40 font-mono">
                    <span>MAX: {filters.maxDist}mi</span>
                  </div>
                  <input
                    type="range"
                    min="0"
            ...
            <button
              onClick={() => setFilters({ startDate: '', endDate: '', minDist: 0, maxDist: 100 })}
              className="col-span-2 py-2 text-[10px] font-bold text-car-accent uppercase tracking-widest hover:text-white transition-colors"
            >
              Reset Filters
            </button>
          </motion.div>
        ...
        <div className="space-y-3">
          {filteredTrips.length === 0 ? (
            <div className="p-8 text-center bg-black/20 rounded-2xl border border-white/5">
              <p className="text-xs text-white/40">No trips match current filters.</p>
            </div>
          ) : (
            filteredTrips.map(trip => (
              <div key={trip.id} className="bg-white/5 rounded-2xl border border-white/5 overflow-hidden">
                <div 
                  onClick={() => setExpandedTripId(expandedTripId === trip.id ? null : trip.id)}
                  className="p-4 flex items-center justify-between cursor-pointer hover:bg-white/5 transition-colors"
                >
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-car-accent/10 flex items-center justify-center text-car-accent">
                      <Navigation size={20} />
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        <span className="text-[10px] text-white/40 font-mono flex items-center gap-1">
                          <Clock size={10} /> {Math.round((trip.endTime! - trip.startTime) / 60000)}m
                        </span>
                        <span className="text-[10px] text-white/40 font-mono flex items-center gap-1">
                          <Ruler size={10} /> {trip.distance.toFixed(1)}mi
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  >
                    <div className="pt-4 space-y-3">
                      <div className="flex justify-between items-center bg-black/40 p-3 rounded-xl border border-white/5">
                        <span className="text-[10px] font-bold text-white/40 uppercase">Avg Damage Score</span>
                        <span className={cn(
                          "text-sm font-bold font-mono",
                          trip.averageDamageScore < 20 ? "text-car-success" : 
                          trip.averageDamageScore < 50 ? "text-car-warning" : "text-car-danger"
                        )}>
                      ...
                      <button
                        onClick={() => {
                          ...
                          // Note: In a real app we'd trigger a map view of this specific trip
                          // For now we'll just center map on start
                          if (start && map) {
                            map.panTo({ lat: start.lat, lng: start.lng });
                            map.setZoom(14);
                          }
                        }}
                        className="w-full py-2 bg-car-accent/10 text-car-accent rounded-xl text-[10px] font-bold uppercase tracking-widest border border-car-accent/20 hover:bg-car-accent/20 transition-all"
                      >
                        Pin to Primary Map
                      </button>
                    </div>
